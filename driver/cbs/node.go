...
import (
	...
	"github.com/von1994/CSIFramework/driver/util"
)
...
func newNodeMetaData() (*nodeMetaData) {
	nodeId, nodeZone := util.GetNodeName()
	return &nodeMetaData{
		NodeId: nodeId,
		Zone:   nodeZone,
	}
...
func newCbsNode(volumeAttachLimit int64) (*cbsNode, error) {
	node := cbsNode{
		mounter: mount.SafeFormatAndMount{
			Interface: mount.New(""),
			Exec:      exec.New(),
...
// NodeStageVolume 将attached的块存储暴露为文件系统挂载点
//  @receiver node
//  @param ctx
//  @param req
